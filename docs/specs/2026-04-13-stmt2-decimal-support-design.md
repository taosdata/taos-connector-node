...
2. **`bind()` 阶段**：在 `WsStmt2.bind()` 中，根据 `this.fields`（来自 `stmt2_prepare` 响应）的 `field_type`，将 DECIMAL 覆写为实际的 DECIMAL64 (21) 或 DECIMAL (17)。

**各模式行为：**

- **Insert 全绑定**：`bind()` 中直接通过 `this.fields[j].field_type` 覆写（索引 j 一一对应）。
- **Insert 非全绑定**：`bind()` 中从 `this.fields` 提取 COL 类型字段，按序匹配 `paramsArray._fieldParams` 索引进行覆写。
...
WsStmt2.bind() 根据 this.fields 覆写 decimal 类型：
  ├── Insert 全绑定：this.fields[j].field_type → 覆写 columnType
  ├── Insert 非全绑定：COL 字段按序匹配 → 覆写 columnType
...
**在 `bind()` 中覆写 decimal 类型**：

```typescript
// Insert 全绑定路径（paramsArray.getBindCount() == this.fields.length）
// 在处理每个 fieldParam 时：
let actualColumnType = fieldParam.columnType;
if (fieldParam.columnType === TDengineTypeCode.DECIMAL &&
    this.fields[j].field_type !== undefined) {
    actualColumnType = this.fields[j].field_type;
    fieldParam.columnType = actualColumnType;
}

// Insert 非全绑定路径
// 在 setParams 之前，按 COL 字段顺序覆写 decimal 类型：
if (this._isInsert && this.fields && paramsArray._fieldParams) {
    ...
    for (let i = 0; i < paramsArray._fieldParams.length; i++) {
        const fp = paramsArray._fieldParams[i];
        if (fp && fp.columnType === TDengineTypeCode.DECIMAL &&
            colFields[i]?.field_type) {
            fp.columnType = colFields[i].field_type;
        }
    }
}
```
