...
export class Result {
  ...
  constructor(res: IResult, commands?: string) {
    let meta_list_length =
      res.column_meta == undefined ? 0 : res.column_meta.length;
    if (res.code == 0) {
      this._code = res.code;
      if (
        res.data &&
        res.column_meta &&
        res.column_meta[0][0] === 'affected_rows'
      ) {
  ...
  private _initData(
    resData: Array<Array<any>>,
    meta: Array<Array<any>>,
    rows: number
  ): void {
    if ((resData.length = 0)) {
      this._data = [[]];
  ...
  private _prettyStr(fields: Array<meta>, data: Array<Array<any>>): string {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      if (fields[i].typeName === 'VARCHAR' || fields[i].typeName === 'NCHAR') {
        ...
        colSize.push(
          Math.max(
            fields[i].columnName.length,
            suggestedMinWidths[fields[i].size]
          )
        ); // max(column_name.length,suggest_column_with_suggestion)
      }
      // console.log(colSize)
    }
    colName.forEach((name, i) => {
      colStr +=
        this._fillEmpty(Math.floor(colSize[i] / 2 - name.length / 2)) +
        name.toString() +
        this._fillEmpty(Math.ceil(colSize[i] / 2 - name.length / 2)) +
        ' | ';
    });
    ...
    data.forEach((row) => {
      ...
      row.forEach((cell, index) => {
        ...
        rowStr +=
          this._fillEmpty(colSize[index] - cell.toString().length) + ' | ';
      });
    });
