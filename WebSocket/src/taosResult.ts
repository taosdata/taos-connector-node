    ...
    for (let i = 0; i < fetchResponse.rows; i++) {
      ...
      for (let j = 0; j < metaList.length; j++) {
        ...
        if (isVarType == ColumnsBlockType.SOLID) {
          ...
          let bitMapArr = dataBuffer.slice(
            colBlockHead,
            colBlockHead + bitMapSize
          );
          ...
          let bitFlag =
            (new DataView(bitMapArr).getUint8(byteArrayIndex) &
              (1 << bitwiseOffset)) >>
            bitwiseOffset;
          ...
          colBlockHead =
            colBlockHead +
            bitMapSize +
            new DataView(dataBuffer, INT_32_SIZE * j, INT_32_SIZE).getInt32(
              0,
              true
            );
        } else {
          // if null check
          let varOffset = new DataView(
            dataBuffer,
          ...
          if (varOffset == -1) {
            ...
            colBlockHead =
              ...
              new DataView(dataBuffer, j * INT_32_SIZE, INT_32_SIZE).getInt32(
                0,
                true
              );
          } else {
            ...
            let dataLength = new DataView(dataBuffer, colDataHead, 2).getInt16(
              0,
              true
            );
...
function readSolidData(
  ...
  switch (meta.type) {
    ...
    case TDengineTypeCode['FLOAT']: {
      return parseFloat(
        new DataView(dataBuffer, colDataHead, 4).getFloat32(0, true).toFixed(5)
      );
    }
    case TDengineTypeCode['DOUBLE']: {
      return parseFloat(
        new DataView(dataBuffer, colDataHead, 8).getFloat64(0, true).toFixed(15)
      );
    }
...
function readVarchar(
  ...
  data += new TextDecoder().decode(
    dataBuffer.slice(colDataHead, colDataHead + length)
  );
  return data;
}
